...
const LineUpScreen = () => {
    // Estado para armazenar TODOS os eventos do Firebase
    const [allEvents, setAllEvents] = useState<Evento[]>([]);
    // programacaoDia agora será uma lista filtrada (pelo dia ou pela busca)
    const [programacaoExibida, setProgramacaoExibida] = useState<Evento[]>([]);
    ...
    // Lógica para buscar TODOS os eventos (agora em allEvents)
    useEffect(() => {
        ...
        const unsubscribeLineup = onValue(lineupRef, (snapshot) => {
            ...
            setAllEvents(eventosCarregados);
        });
        return () => unsubscribeLineup();
    }, []); // Executa apenas uma vez para carregar todos os eventos

    // Efeito para filtrar a programação exibida com base no dia selecionado OU no termo de busca
    useEffect(() => {
        let eventosParaExibir: Evento[] = [];

        if (searchTerm.length >= 3) {
            // Se há um termo de busca, filtra em allEvents
            const lowerCaseSearchTerm = searchTerm.toLowerCase();
            eventosParaExibir = allEvents.filter(evento => {
                const formattedDataMomento = moment(evento.dataMomento, 'DD-MM-YYYY').format('DD/MM/YYYY').toLowerCase();
                return (
                    evento.nomeBanda.toLowerCase().includes(lowerCaseSearchTerm) ||
                    evento.horaInicio.toLowerCase().includes(lowerCaseSearchTerm) ||
                    evento.local.toLowerCase().includes(lowerCaseSearchTerm) ||
                    formattedDataMomento.includes(lowerCaseSearchTerm)
                );
            }).sort((a, b) => {
                // Ordenar por data e depois por hora ao buscar em todos os eventos
                const dateA = moment(a.dataMomento, 'DD-MM-YYYY').toDate().getTime();
                const dateB = moment(b.dataMomento, 'DD-MM-YYYY').toDate().getTime();
                if (dateA !== dateB) {
                    return dateA - dateB;
                }
            ...
            // Se não há termo de busca, filtra por dia selecionado
            eventosParaExibir = allEvents.filter((evento) => {
        ...
        setProgramacaoExibida(eventosParaExibir);
    }, [allEvents, diaSelecionado, searchTerm]); // Dependências atualizadas
    ...
    const avancarDia = () => {
        setSearchTerm('');
        setDiaSelecionado(moment(diaSelecionado).add(1, 'day').toDate());
    };
    const voltarDia = () => {
        setSearchTerm('');
        setDiaSelecionado(moment(diaSelecionado).subtract(1, 'day').toDate());
    };
    ...
    const renderLiveStreamLinkItem = ({ item }: { item: Locais }) => {
        const getPlatformIcon = (url: string) => {
            ...
            } else if (url.includes('youtube.com')) { // Mais genérico para YouTube
    ...
    if (!fundoAppReady || gettingUserLocation) {
        return (
            ...
            <SafeAreaView style={styles.safeAreaContainer}>
                <View style={styles.container}>
                    ...
                    <TextInput
                        ...
                        placeholder="Buscar por banda, local, hora ou data..."
                ...
                <View style={styles.mapOuterContainer}>
                    <View style={styles.mapContainer}>
                        <MapView style={styles.map} initialRegion={mapRegion} region={mapRegion}>
                            ...
                            {/* O filtro de eventos no mapa ainda usa programacaoDia, para exibir apenas os do dia atual */}
                            {allEvents // Alterado para allEvents para que o mapa possa mostrar todos os marcadores, mas o filtro ainda é por dia
                                .filter(evento => {
                                    const localDoEvento = locaisData.find((local) => local.descricao.toLowerCase().trim() === evento.local.toLowerCase().trim());
                                    // Filtra apenas eventos que possuem localização válida E que são do dia selecionado
                                    const dataEvento = moment(evento.dataMomento, 'DD-MM-YYYY');
                                    const dataSelecionada = moment(diaSelecionado);
                                    return localDoEvento && typeof localDoEvento.latitude === 'number' && typeof localDoEvento.longitude === 'number' && dataEvento.isSame(dataSelecionada, 'day');
                                })
                                .map((evento) => {
                                    const localDoEvento = locaisData.find((local) => local.descricao.toLowerCase().trim() === evento.local.toLowerCase().trim())!;
...
const styles = StyleSheet.create({
    ...
    sectionTitle: { // Mantido caso queira usar para outros títulos
        fontSize: 18,
        fontWeight: 'bold',
        color: '#FFFFFF',
        marginBottom: 10,
